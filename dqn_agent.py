...
class DQNAgent(object):
    ...
    def create_variables_for_target_network_update(self):
        with tf.name_scope("target_network_update"):
            target_ops = []
            q_network_variables = tf.get_collection(tf.GraphKeys.VARIABLES, scope="q_network")
            target_network_variables = tf.get_collection(tf.GraphKeys.VARIABLES, scope="target_network")
    ...
    def create_summaries(self):
        ...
        weights = tf.get_collection(tf.GraphKeys.VARIABLES, scope="q_network")
        for w in weights:
            w_summary = tf.histogram_summary("q_weights/" + w.name, w)
